{pkgs, ...}: let
  ...
  waybarKeyboardBacklight = pkgs.writeShellScriptBin "waybar-keyboard-backlight" ''
    set -euo pipefail
    ...
    detect_kbd_device() {
      ...
      dev=$(
        brightnessctl -l \
          ...
          | grep -E "(kbd_backlight|::kbd_backlight|keyboard_backlight)" \
          | head -n1 || true
      )
      printf '%s\n' "$dev"
    }

    device="$(detect_kbd_device)"

    if [[ "''${1:-}" == "--has-device" ]]; then
      [[ -n "$device" ]]
      exit $?
    fi
    ...
    while getopts o: opt; do
      case "$opt" in
        o)
          case "$OPTARG" in
            i) [[ -n "$device" ]] && brightnessctl -d "$device" set +5% >/dev/null ;;
            d) [[ -n "$device" ]] && brightnessctl -d "$device" set 5%- >/dev/null ;;
          esac
          ;;
      esac
    done
    ...
    if [[ -z "$device" ]]; then
      echo '{"text":"󰌌 N/A","tooltip":"No keyboard backlight device exposed by kernel/firmware on this host"}'
      exit 0
    fi

    pct="$(brightnessctl -d "$device" -m | awk -F, '{gsub(/%/, "", $4); print $4}')"
    cur="$(brightnessctl -d "$device" g)"
    max="$(brightnessctl -d "$device" m)"
    printf '{"text":"󰌌 %s%%","tooltip":"Keyboard backlight (%s): %s/%s"}\n' "$pct" "$device" "$cur" "$max"
  '';
  ...
  programs.waybar = {
    ...
    settings = [
      {
        ...
        "custom/keyboard_backlight" = {
          return-type = "json";
          "exec-if" = "${waybarKeyboardBacklight}/bin/waybar-keyboard-backlight --has-device";
          exec = "${waybarKeyboardBacklight}/bin/waybar-keyboard-backlight";
          interval = 5;
          "on-scroll-up" = "${waybarKeyboardBacklight}/bin/waybar-keyboard-backlight -o i";
          "on-scroll-down" = "${waybarKeyboardBacklight}/bin/waybar-keyboard-backlight -o d";
          tooltip = true;
        };
        ...
        "hyprland/workspaces#rw" = {
          ...
          "window-rewrite" = {
            ...
            "title<.*youtube.*>" = " ";
            "class<de.schmidhuberj.tubefeeder>" = " ";
            "class<vlc>" = "󰕼 ";
        ...
        "custom/screenrec" = {
          ...
          interval = 1;
          tooltip = true;
          "on-click" = "screenrec region";
          "on-click-right" = "screenrec screen";
        };
    ...
    in ''
      ...
      * {
      ...
      /* Tray icons can inherit an invisible color; force a visible one */
      #tray { color: @sapphire; }
      #tray * { color: @sapphire; }
